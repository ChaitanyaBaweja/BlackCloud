 ...
 ****************************************************/
/*
Code modified by Chaitanya Baweja, University of Delhi
This program is used to test a SIM800 based GSM Module.

Open up the serial console on the Arduino at 115200 baud to interact with FONA
To set a GPRS APN, username, and password scroll down to
...
SoftwareSerial *gsmSerial = &sS;

Adafruit_FONA gsm = Adafruit_FONA(RST);
...
void setup() {
  while (!Serial);
  Serial.begin(115200);
  Serial.println(F("GSM basic test"));
  ...
  uint8_t imeiLen = gsm.getIMEI(imei);
  if (imeiLen > 0) {
  ...
  gsm.setGPRSNetworkSettings(F("airtelgprs.com"), F(""), F(""));
...
void loop() {
  ...
  switch (command) {
    ...
    case 'C': {
        // read the CCID
        gsm.getSIMCCID(replybuffer);  // make sure replybuffer is at least 21 bytes!
    ...
    case 'n': {
        ...
        uint8_t n = gsm.getNetworkStatus();
    ...
    case 'H': {
        ...
        if (! gsm.setAudio(FONA_HEADSETAUDIO)) {
          Serial.println(F("Failed"));
        } else {
          Serial.println(F("OK!"));
        }
        gsm.setMicVolume(FONA_HEADSETAUDIO, 15);
        break;
      }
    case 'e': {
        // Set External output
        if (! gsm.setAudio(FONA_EXTAUDIO)) {
          Serial.println(F("Failed"));
        } else {
          Serial.println(F("OK!"));
        }

        gsm.setMicVolume(FONA_EXTAUDIO, 10);
        break;
      }
    ...
    case 'T': {
        ...
        if (! gsm.playToolkitTone(kittone, 1000)) {
          Serial.println(F("Failed"));
        } else {
          Serial.println(F("OK!"));
        }
    ...
    case 'f': {
        ...
        if (gsm.FMradio(true, FONA_HEADSETAUDIO)) {
          Serial.println(F("Opened"));
        }
        if (! gsm.tuneFMradio(station)) {
          Serial.println(F("Failed"));
        } else {
    ...
    case 'q': {
        ...
        uint16_t station = readnumber();
        Serial.println();
        int8_t level = gsm.getFMSignalLevel(station);
        if (level < 0) {
    ...
    case 'A': {
        // get call status
        int8_t callstat = gsm.getCallStatus();
        switch (callstat) {
    ...
    case 'u': {
      ...
      uint16_t ussdlen;
      if (!gsm.sendUSSD(message, replybuffer, 250, &ussdlen)) { // pass in buffer and max len!
        Serial.println(F("Failed"));
      } else {
        Serial.println(F("Sent!"));
    ...
    case 'Y': {
        ...
        if (!gsm.enableNTPTimeSync(true, F("pool.ntp.org")))
          Serial.println(F("Failed to enable"));
        break;
      }
    ...
    case 't': {
        ...
        gsm.getTime(buffer, 23);  // make sure replybuffer is at least 23 bytes!
    ...
    case 'l': {
        ...
        if (!gsm.getGSMLoc(&returncode, replybuffer, 250))
          Serial.println(F("Failed!"));
        if (returncode == 0) {
          Serial.println(replybuffer);
        } else {
    ...
    case 'w': {
        ...
        Serial.println(F("****"));
        if (!gsm.HTTP_GET_start(url, &statuscode, (uint16_t *)&length)) {
          Serial.println("Failed!");
          break;
        }
        while (length > 0) {
    ...
    case 'W': {
        ...
        Serial.println(F("****"));
        if (!gsm.HTTP_POST_start(url, F("text/plain"), (uint8_t *) data, strlen(data), &statuscode, (uint16_t *)&length)) {
          Serial.println("Failed!");
          break;
        }
        while (length > 0) {
